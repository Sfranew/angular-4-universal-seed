 ...
 */
connectDB = function()  {
	const dbConnection_maeci_db = mongoose.createConnection('mongodb://' + properties.maeci_db_dbUrl, function(err){
		if(err) {
			logger.error(err);
			setTimeout(function() {
		...
		{
		    exports.connection = dbConnection_maeci_db;
			logger.info("MongoDB connected at: " + properties.maeci_db_dbUrl);
			
			//START IMPORT RESOURCE

			require('./maeci_db_schema.js');
            require('../security/userResource.js');

			require('../resource/maeci_db/Immobile.js');
